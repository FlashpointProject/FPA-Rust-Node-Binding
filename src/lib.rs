...
use flashpoint_archive::{
    game::{
        ext::ExtensionInfo, search::{GameFilter, GameSearch, PageTuple, ParsedInput}, AdditionalApp, Game, GameRedirect, PartialGame
    },
    game_data::{GameData, PartialGameData},
    platform::PlatformAppPath,
    tag::{PartialTag, Tag, TagSuggestion},
    tag_category::{PartialTagCategory, TagCategory},
    update::{RemoteCategory, RemoteDeletedGamesRes, RemoteGamesRes, RemotePlatform, RemoteTag},
    util::ContentTreeNode,
    FlashpointArchive,
};
use napi::{
    threadsafe_function::{
        ErrorStrategy::CalleeHandled, ThreadSafeCallContext, ThreadsafeFunction,
    },
...
impl FlashpointNode {
    ...
    #[napi]
    pub async fn search_games_index(
        &self,
        mut search: GameSearch,
        limit: Option<i64>,
    ) -> Result<Vec<PageTuple>> {
        self.flashpoint
            .search_games_index(&mut search, limit)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn search_games_random(&self, search: GameSearch, count: i64) -> Result<Vec<Game>> {
        self.flashpoint
            .search_games_random(&search, count)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn search_tag_suggestions(
        ...
        self.flashpoint
            .search_tag_suggestions(&partial, blacklist)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn search_platform_suggestions(&self, partial: String) -> Result<Vec<TagSuggestion>> {
        self.flashpoint
            .search_platform_suggestions(&partial)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn find_game_data_by_id(&self, game_data_id: i64) -> Result<Option<GameData>> {
        self.flashpoint
            .find_game_data_by_id(game_data_id)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn new_tag_filter_index(&self, mut search: GameSearch) -> Result<()> {
        self.flashpoint
            .new_tag_filter_index(&mut search)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }

    #[napi]
    pub async fn find_all_game_developers(
        &self,
        search: Option<GameSearch>,
    ) -> Result<Vec<String>> {
        self.flashpoint
            .find_all_game_developers(search)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }

    #[napi]
    pub async fn find_all_game_publishers(
        &self,
        search: Option<GameSearch>,
    ) -> Result<Vec<String>> {
        self.flashpoint
            .find_all_game_publishers(search)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn clear_playtime_tracking_by_id(&self, game_id: String) -> Result<()> {
        self.flashpoint
            .clear_playtime_tracking_by_id(&game_id)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn find_all_game_application_paths(&self) -> Result<Vec<String>> {
        self.flashpoint
            .find_all_game_application_paths()
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn force_games_active_data_most_recent(&self) -> Result<()> {
        self.flashpoint
            .force_games_active_data_most_recent()
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn create_game_redirect(&self, src_id: String, dest_id: String) -> Result<()> {
        self.flashpoint
            .create_game_redirect(&src_id, &dest_id)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn delete_game_redirect(&self, src_id: String, dest_id: String) -> Result<()> {
        self.flashpoint
            .delete_game_redirect(&src_id, &dest_id)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn update_apply_redirects(&self, redirects: Vec<GameRedirect>) -> Result<()> {
        self.flashpoint
            .update_apply_redirects(redirects)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }
    ...
    pub async fn new_custom_id_order(&self, custom_id_order: Vec<String>) -> Result<()> {
        self.flashpoint
            .new_custom_id_order(custom_id_order)
            .await
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }

    #[napi]
    pub fn register_extension(&mut self, ext_info: ExtensionInfo) -> Result<()> {
        self.flashpoint
            .register_extension(ext_info)
            .map_err(|e| Error::new(Status::GenericFailure, e))
    }

    #[napi]
    pub fn parse_user_search_input(&self, input: String) -> ParsedInput {
        self.flashpoint
...
pub async fn gen_content_tree(root: String) -> Result<ContentTreeNode> {
    flashpoint_archive::generate_content_tree(&root)
        .map_err(|e| Error::new(Status::GenericFailure, e))
}
...
pub fn logger_susbcribe(callback: JsFunction) -> Result<()> {
    ...
    let tsfn: ThreadsafeFunction<String, CalleeHandled> = callback
